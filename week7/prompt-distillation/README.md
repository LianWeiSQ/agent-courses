...
> Rust 重构版路径：`week7/prompt-distillation`

## 这节课要解决什么

把长提示中的策略压缩进模型行为，降低推理成本。
...
```bash
cargo run -p agent-course-week7-prompt-distillation
```

单独测试这一节：

```bash
cargo test -p agent-course-week7-prompt-distillation
```
