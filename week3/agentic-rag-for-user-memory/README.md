...
> Rust 重构版路径：`week3/agentic-rag-for-user-memory`

## 这节课要解决什么

把 Agentic RAG 用在跨会话用户记忆检索。
...
```bash
cargo run -p agent-course-week3-agentic-rag-for-user-memory
```

单独测试这一节：

```bash
cargo test -p agent-course-week3-agentic-rag-for-user-memory
```
