...
> Rust 重构版路径：`week3/structured-knowledge-extraction`

## 这节课要解决什么

从非结构化文本抽取因素、规则和可复用判断逻辑。
...
```bash
cargo run -p agent-course-week3-structured-knowledge-extraction
```

单独测试这一节：

```bash
cargo test -p agent-course-week3-structured-knowledge-extraction
```
