...
> Rust 重构版路径：`week3/contextual-retrieval-for-user-memory`

## 这节课要解决什么

结合结构化事实卡和上下文检索，构造双层记忆。
...
```bash
cargo run -p agent-course-week3-contextual-retrieval-for-user-memory
```

单独测试这一节：

```bash
cargo test -p agent-course-week3-contextual-retrieval-for-user-memory
```
