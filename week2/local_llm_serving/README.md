...
> Rust 重构版路径：`week2/local_llm_serving`

## 这节课要解决什么

理解本地模型服务、Chat Template、流式输出和工具调用之间的关系。
...
```bash
cargo run -p agent-course-week2-local-llm-serving
```

单独测试这一节：

```bash
cargo test -p agent-course-week2-local-llm-serving
```
