...
> Rust 重构版路径：`week2/user-memory-evaluation`

## 这节课要解决什么

为用户记忆系统设计准确性、相关性和覆盖度评估。
...
```bash
cargo run -p agent-course-week2-user-memory-evaluation
```

单独测试这一节：

```bash
cargo test -p agent-course-week2-user-memory-evaluation
```
