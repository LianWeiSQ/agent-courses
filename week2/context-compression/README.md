...
> Rust 重构版路径：`week2/context-compression`

## 这节课要解决什么

比较摘要、抽取和结构化压缩，学习在 token 预算内保留关键信息。
...
```bash
cargo run -p agent-course-week2-context-compression
```

单独测试这一节：

```bash
cargo test -p agent-course-week2-context-compression
```
