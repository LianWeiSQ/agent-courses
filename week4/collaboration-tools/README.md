...
> Rust 重构版路径：`week4/collaboration-tools`

## 这节课要解决什么

连接浏览器、人类审批、通知和定时器，让 Agent 能协作。
...
```bash
cargo run -p agent-course-week4-collaboration-tools
```

单独测试这一节：

```bash
cargo test -p agent-course-week4-collaboration-tools
```
