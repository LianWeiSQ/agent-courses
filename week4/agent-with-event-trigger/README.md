...
> Rust 重构版路径：`week4/agent-with-event-trigger`

## 这节课要解决什么

用事件、HTTP API 和工具编排构建异步 Agent 系统。
...
```bash
cargo run -p agent-course-week4-agent-with-event-trigger
```

单独测试这一节：

```bash
cargo test -p agent-course-week4-agent-with-event-trigger
```
