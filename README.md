...
这个仓库按 `week1` 到 `week8` 组织，每个项目目录都围绕一个 Agent 能力展开：先讲它解决什么问题，再给课堂演示命令，最后给出核心概念和练习方向。

## 重要约定

- 课程目录按 `week1/...` 到 `week8/...` 组织。
- 每个项目目录都包含一个可运行的课堂演示。
- 课堂演示可以直接用 `cargo run -p ...` 启动。
- 每个项目都有 `README.md`。
- 额外的 Markdown 文章也围绕 Agent 能力、课堂讨论和练习任务组织。
...
当前课程以 Agent 核心概念和本地可运行演示为优先。外部模型 API、浏览器、训练集和大文件资源没有直接放进仓库；它们在每节课的练习方向里作为下一步接入点保留。
